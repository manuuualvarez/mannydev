...
description: Generate Vitest + React Testing Library tests for manuelalvarez.cloud frontend components, hooks, and utilities. Triggers on testing, spec files, coverage, Vitest, RTL, unit tests, integration tests, or write/review test requests.
---

# Frontend Testing Skill - manuelalvarez.cloud

This skill enables Claude to generate high-quality, comprehensive frontend tests for the manuelalvarez.cloud project following established conventions and best practices.

> **⚠️ Use Vitest mock/timer APIs (`vi.*`).**

## Pre-Testing: Use Next.js MCP

**ANTES de escribir tests, usa el MCP de Next.js DevTools:**

1. **`get_errors`** - Verificar que no hay errores de build/TypeScript
2. **`get_project_metadata`** - Entender la estructura del proyecto
3. **`get_page_metadata`** - Entender qué componentes tiene cada página

Si hay errores de build o TypeScript, corregirlos ANTES de escribir tests.
...
# get_errors → Should return no errors before testing
```
...
**Usar MCP de Next.js DevTools para verificación:**

```
1. ANTES de escribir tests:
   - get_errors → Verificar no hay errores de build/TypeScript

2. DESPUÉS de escribir tests:
   - npm run test → Ejecutar tests
   - get_errors → Verificar que no se introdujeron errores
```
